...
for file in "$parent_folder"*/; do
    ...
    bedtools getfasta -fi /path/to/rRNA/rRNA.fasta -bed "$sample.rna.tmp.bed" -nameOnly -tab -s | awk -F"\t" '{OFS="\t"; if(length($2)==5){a=toupper($2);gsub(/T/,"U",a); if(a~/[GAU][AG]AC[ACU]/) {m="DRACH"} else {m="nonDRACH"}; print $0"\t"m}}' | perl -pe 's/\([+-]\)//g'  | awk -F"\t" '{OFS="\t"; split($1, info, "="); print info[1],info[2]-1,info[2],$1"="$2"="$3,info[6]/info[4],info[3]}' > "$sample.rRNA.Conversions.bed"
...
cd /path/to/result/bed/result/
mkdir final_output

for gen in WT KO; do 
    for treatment in UT BT2 BT4; do 
        for rep in 1 2; do 
            input_file="/path/to/result/bed/result/${gen}_${treatment}_${rep}.eTAMInput.tsv"
            output_file="/path/to/result/bed/result/final_output/${gen}_${treatment}_${rep}.ivtcount.table.out.txt"
